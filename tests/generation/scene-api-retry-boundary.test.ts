...
describe('scene API retry boundary', () => {
  ...
  it('maps an upstream 401 from the scene-content route to INVALID_CREDENTIALS', async () => {
    vi.resetModules();
    ...
    expect(body).toMatchObject({
      ...
      error: 'Authentication failed. Please verify your API key for the model provider.',
    });
  });

  it('collapses an upstream 503 from the scene-content route to 502', async () => {
    vi.resetModules();
  ...
  it('maps an upstream 401 from the scene-actions route to INVALID_CREDENTIALS', async () => {
    vi.resetModules();
    const unauthorized = Object.assign(new Error('provider key rejected'), { statusCode: 401 });
    mocks.generateSceneActions.mockImplementation(async (_outline, _content, aiCall) => {
      await aiCall('system', 'user');
  ...
  it('collapses an upstream 503 from the scene-actions route to 502', async () => {
    vi.resetModules();
    const unavailable = Object.assign(new Error('provider overloaded'), { statusCode: 503 });
    mocks.generateSceneActions.mockImplementation(async (_outline, _content, aiCall) => {
      await aiCall('system', 'user');
