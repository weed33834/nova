...
Return a JSON object with this exact structure:
{
  "agents": [
    ...
    const context = `stage="${stageName ?? 'unknown'}", model=${modelString ?? 'unknown'}"`;
    log.error(`Agent profiles generation failed [${context}]:`, error);
    // Consistent with scene-content/scene-actions: let llmApiError map the
    // error to the appropriate code (INVALID_CREDENTIALS / MODEL_NOT_FOUND /
    // RATE_LIMITED / UPSTREAM_ERROR / INTERNAL_ERROR) with a sanitized message
