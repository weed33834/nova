...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const context = `provider=${ttsProviderId ?? 'unknown'}, voice=${ttsVoice ?? 'unknown'}, audioId=${audioId ?? 'unknown'}`;
    log.error(`TTS generation failed [${context}]:`, error);
    if (error instanceof TTSRateLimitError) {
      return apiError(
        'RATE_LIMITED',
        429,
        'TTS rate limit reached. Please try again shortly.',
        undefined,
        context,
      );
    }
    // Upstream LLM/provider errors carry an HTTP status — let llmApiError
    // sanitize the message (no raw provider bodies/URLs/credentials leaked).
    if (upstreamStatusFromError(error) !== undefined) {
      return llmApiError(error, context);
    }
    // Non-LLM errors (filesystem, config, etc.) — generic message; the real
    // error was logged above for server-side debugging.
    return apiError(
      'GENERATION_FAILED',
      500,
