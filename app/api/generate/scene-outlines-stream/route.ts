...
function sanitizeNonTaskEngineOutline(outline: SceneOutline): SceneOutline {
  ...
  // procedural-skill is gated behind taskEngineMode to protect ordinary Nova generation.
  return {
    ...outline,
    type: 'interactive',
    widgetType: 'diagram',
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    // courseFormat overrides: 'text-only' forces all media off,
    // 'ppt-audio' forces video off — regardless of the global header flags.
    const headerImageEnabled = req.headers.get('x-image-generation-enabled') === 'true';
    const headerVideoEnabled = req.headers.get('x-video-generation-enabled') === 'true';
    const effectiveFlags = getEffectiveMediaFlags(requirements.courseFormat, {
      imageGenerationEnabled: headerImageEnabled,
      videoGenerationEnabled: headerVideoEnabled,
      // ttsEnabled is not used in outline prompt snippet injection
    });
    const imageGenerationEnabled = effectiveFlags.imageEnabled;
    const videoGenerationEnabled = effectiveFlags.videoEnabled;
    ...
    return apiError('INTERNAL_ERROR', 500, 'Unable to generate course outlines');
  }
}
