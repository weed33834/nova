/**
 * Nova Agent — SSE transport endpoint.
 *
...
interface AgentEditBody {
  ...
  selection?: string[];
  /**
   * MCP server configurations (Phase D). The client forwards its persisted
   * `mcpServersConfig` so the server can spawn/connect external MCP servers
   * and adapt their tools into the agent's toolset. Empty/absent = no MCP
   * tools. Stdio transport requires a long-lived process (standalone only);
   * HTTP transport works in any deployment. A failing server is non-fatal.
   */
...
export async function POST(req: NextRequest) {
  ...
  // ─── MCP tools (Phase D) ───
  // Connect (idempotently) to any configured MCP servers the client forwarded,
  // adapt their tools into pi AgentTool objects, and merge them into the
  // toolset + allowlist. `connectAll` short-circuits unchanged servers, so the
  // per-request cost is one Map scan when the config is stable. A failing
  // server contributes no tools (non-fatal) — see MCPClientManager.connectOne.
  const mcpManager = getMCPClientManager();
  const requestedMcpConfig = body.mcpServersConfig ?? {};
  const mcpConfig: MCPServersConfig = {};
  for (const [id, server] of Object.entries(requestedMcpConfig)) {
    if (server?.transport !== 'http' || typeof server.url !== 'string') continue;
    if (await validateUrlForSSRF(server.url)) continue;
    mcpConfig[id] = server;
  }
  const hasMcpConfig = Object.keys(mcpConfig).length > 0;
  let mcpTools: typeof tools = [];
  let combinedAllowlist: ReadonlySet<string> = V0_ALLOWLIST;
  if (hasMcpConfig) {
    await mcpManager.connectAll(mcpConfig);
    const adapted = adaptAllMCPTools(mcpManager);
    mcpTools = adapted.tools;
    combinedAllowlist = new Set([...V0_ALLOWLIST, ...adapted.names]);
