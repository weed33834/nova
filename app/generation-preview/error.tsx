...
/**
 * Route-level error boundary for the generation preview page.
 *
 * Catches rendering errors thrown by `app/generation-preview/page.tsx` and its
...
export default function GenerationPreviewError({
  ...
  useEffect(() => {
    console.error('[GenerationPreviewError]', error, error.digest ?? '');
  }, [error]);

  return (
    <div className="flex min-h-[100dvh] w-full flex-col items-center justify-center bg-gradient-to-b from-slate-50 to-slate-100 p-4 dark:from-slate-950 dark:to-slate-900">
      <div className="w-full max-w-md space-y-6 text-center">
        ...
        <div className="space-y-2">
          <h2 className="text-lg font-semibold text-foreground">
            {t('errors.generationFailedToLoad')}
          </h2>
          <p className="text-sm text-muted-foreground">{t('errors.generationFailedToLoadDesc')}</p>
        </div>
