  ...
  const onNew = useCallback(
    async (message: AppendMessage) => {
      ...
      try {
        ...
        // Forward persisted MCP server configs (Phase D) so the server can
        // connect external MCP servers and adapt their tools into the toolset.
        const mcpServersConfig = useSettingsStore.getState().mcpServersConfig;
        const res = await fetch('/api/agent/edit', {
          ...
          body: JSON.stringify({
            ...
            // Omit mcpServersConfig when empty to keep the body lean for users
            // who don't use MCP.
            ...(mcpServersConfig && Object.keys(mcpServersConfig).length > 0
              ? { mcpServersConfig }
              : {}),
          }),
          signal: abort.signal,
