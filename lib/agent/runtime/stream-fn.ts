/**
 * Nova Agent — pi StreamFn adapter (promoted from PoC).
 *
 * Bridges the pi agent loop's LLM call to Nova's existing AI-SDK-based
 ...
 * call through Nova's resolved Vercel `LanguageModel`, then map the AI SDK
...
/** Build a pi `StreamFn` that calls Nova's connector instead of pi-ai providers. */
