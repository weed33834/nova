...
async function downloadToBuffer(url: string): Promise<Buffer> {
  ...
  for (let redirects = 0; redirects <= MAX_DOWNLOAD_REDIRECTS; redirects++) {
    ...
    if (ssrfError) throw new Error(`Unsafe media download URL: ${ssrfError}`);
    ...
    if (!location) throw new Error(`Redirect response missing Location header: ${resp.status}`);
    currentUrl = new URL(location, currentUrl).toString();
  }

  if (!resp) throw new Error('Download did not return a response');
  if (resp.status >= 300 && resp.status < 400) {
    throw new Error(`Too many media download redirects (max ${MAX_DOWNLOAD_REDIRECTS})`);
  }
  ...
  try {
    while (true) {
      ...
      if (totalBytes > DOWNLOAD_MAX_SIZE) {
        await reader.cancel('Media download exceeded size limit');
        throw new Error(`File too large: over ${DOWNLOAD_MAX_SIZE} bytes`);
      }
