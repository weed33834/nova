...
import type { Slide, PPTElementOutline, PPTElementShadow, PPTElementLink } from '@nova/dsl';
...
/** Remote media must not leave a PPTX export waiting on an unavailable host. */
async function fetchRemoteAsset(url: string): Promise<Response> {
    ...
    for (const el of slide.elements) {
      ...
      // ── IMAGE ──
        ...
        if (!isBase64Image(resolvedSrc)) {
          try {
            const resp = await fetchRemoteAsset(resolvedSrc);
            if (!resp.ok) {
              log.warn(
      ...
      // ── VIDEO / AUDIO ──
        ...
        try {
          ...
          if (el.type === 'video') {
            ...
            if (posterUrl) {
              try {
                const posterResp = await fetchRemoteAsset(posterUrl);
                if (!posterResp.ok) {
