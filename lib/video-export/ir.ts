 ...
 *
 * Pure: depends only on `@nova/dsl` (the `SceneType` set) and `zod`.
 */
import { z } from 'zod';
import { SCENE_TYPES } from '@nova/dsl';

/** Manifest `schema` tag — stable across versions; the shape is versioned by {@link VIDEO_TIMELINE_VERSION}. */
export const VIDEO_TIMELINE_SCHEMA = 'nova.videoTimeline';
...
export const VIDEO_TIMELINE_COMPILER = 'nova-video-timeline';
