...
/** P1-1: Error info attached to a failed outline. */
export interface FailedOutlineReason {
  /** Human-readable error message (safe to show to users). */
  reason: string;
  /** Machine-readable error code from ApiErrorCode, if available. */
  errorCode?: string;
  /** Which generation step failed: 'content' | 'actions' | 'tts' | 'outline'. */
  failedStep?: string;
...
interface StageState {
  ...
  /** P1-1: Error reasons for failed outlines, keyed by outline.id.
   *  Additive map — `failedOutlines` still stores SceneOutline[] for backward compat.
   *  UI reads this map to show WHY a scene failed (errorReason, errorCode, failedStep). */
  failedOutlineReasons: Record<string, FailedOutlineReason>;
  ...
  /** Mark an outline as failed. Optional error info is stored in `failedOutlineReasons`.
   *  Backward-compatible: existing callers passing only `outline` still work. */
  addFailedOutline: (
...
const useStageStoreBase = create<StageState>()((set, get) => ({
  ...
  addFailedOutline: (outline, errorReason, errorCode, failedStep) => {
    const existed = get().failedOutlines.some((o) => o.id === outline.id);
    if (existed) return;
    set({
      failedOutlines: [...get().failedOutlines, outline],
      // P1-1: Store error reason if provided, so UI can show WHY a scene failed.
      failedOutlineReasons: errorReason
        ? {
            ...get().failedOutlineReasons,
            [outline.id]: { reason: errorReason, errorCode, failedStep, at: Date.now() },
          }
        : get().failedOutlineReasons,
    });
  },
  ...
  retryFailedOutline: (outlineId) => {
    ...
    set({
      ...
      failedOutlineReasons: remainingReasons,
    });
  },
