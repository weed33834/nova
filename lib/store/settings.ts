...
export interface SettingsState {
  ...
  // MCP actions
  setMCPServersConfig: (config: Record<string, SettingsState['mcpServersConfig'][string]>) => void;
  ...
  persist(
    (set, get) => {
      ...
      return {
        ...
        // MCP servers (empty by default; configured via Settings → MCP)
        mcpServersConfig: {},
        ...
        // MCP actions
        setMCPServersConfig: (config) => set({ mcpServersConfig: config }),
    ...
    },
    {
      ...
      migrate: (persistedState: unknown, version: number) => {
        ...
        // Ensure MCP servers config exists (added in this version)
        if (!state.mcpServersConfig) {
          state.mcpServersConfig = {};
        }
