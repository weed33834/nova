...
// VideoManifest / SlideContent / QuizContent / …) now lives in `@nova/dsl` and
...
import type { Scene as DslScene, SceneContent as DslSceneContent } from '@nova/dsl';
...
export { isSlideContent, isQuizContent } from '@nova/dsl';

// `@nova/dsl` inlines the question-type union on `QuizQuestion.type` rather than
// exporting a named alias; derive it here so editor quiz code can keep importing
// `QuizQuestionType` from `@/lib/types/stage`.
export type QuizQuestionType = import('@nova/dsl').QuizQuestion['type'];
...
// four cases.
export type { SceneContent as SceneContentBase } from '@nova/dsl';
...
export type { Scene as SceneShape } from '@nova/dsl';

/**
 * Interactive content - Interactive web page (iframe).
 *
 * App-level feature surface: kept here rather than in `@nova/dsl` because it
...
export type AppScene = DslScene<Action, SceneContent> & {
   ...
   * annotation only — not part of the `@nova/dsl` Scene contract. Absent on
