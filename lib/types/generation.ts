...
export interface UserRequirements {
  ...
  courseFormat?: CourseFormat; // Output format: video / ppt-audio / text-only
}

/**
 * Course output format — controls which media generation steps run.
 * Stored on Stage so it persists across sessions and survives generation resume.
 *
 * - `video`      → full classroom: images + video + TTS (default)
 * - `ppt-audio`  → PPT + TTS voiceover, no video generation
 * - `text-only`  → text content only, no images / video / TTS
 *
 * Mode acts as a macro override: when a format disables a medium, it wins
 * over the corresponding global setting flag for THIS course only.
 */
export type CourseFormat = 'video' | 'ppt-audio' | 'text-only';
...
export interface WidgetOutline {
  ...
  // knowledge-graph
  graphSubject?: string; // knowledge-graph - subject domain (e.g. "photosynthesis")
  graphLayout?: 'force' | 'hierarchy' | 'radial'; // knowledge-graph - layout preference
}
...
import type { PPTElement, SlideBackground } from '@nova/dsl';
