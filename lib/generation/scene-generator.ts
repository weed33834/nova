...
import type { PPTElement, Slide, SlideBackground, SlideTheme } from '@nova/dsl';
import { normalizeElement } from '@nova/dsl';
...
export interface SceneContentOptions {
  ...
  /**
   * Natural-language edit instruction for whole-slide regeneration (Nova Editor
...
function inferWidgetType(subject: string, concept: string, designIdea: string): WidgetType {
  ...
  if (
    /knowledge.?graph|concept.?map|knowledge.?tree|prerequisite.?chain|知识图谱|知识结构|概念图|概念关系|前置知识|知识树|知识体系/.test(
      text,
    )
  ) {
...
function buildWidgetOutline(
  ...
  switch (widgetType) {
    ...
    case 'knowledge-graph':
      return { ...base, graphSubject: config.conceptName, graphLayout: 'force' };
    default:
      return base;
  }
}
 ...
 * owned by the DSL contract — `normalizeElement` from `@nova/dsl` — rather
...
async function generateSlideContent(
  ...
  // EDIT MODE (Nova Editor agent `regenerate_scene`): when an edit instruction
...
export async function generateWidgetContent(
  ...
  switch (widgetType) {
    ...
    case 'knowledge-graph':
      promptId = PROMPT_IDS.KNOWLEDGE_GRAPH_CONTENT;
      variables = {
        title: outline.title,
        graphSubject: widgetOutline.graphSubject || widgetOutline.concept || outline.title,
        description: outline.description,
        keyPoints: (outline.keyPoints || []).join('\n'),
        graphLayout: widgetOutline.graphLayout || 'force',
        languageDirective: languageDirective || '',
      };
      break;
...
export function extractInteractiveElements(html: string): string {
  ...
  for (const match of dom.matchAll(tagRegex)) {
    ...
    const dataNodeId = attrs['data-node-id'];
    const dataEdgeId = attrs['data-edge-id'];
    ...
    if (id && !seenIds.has(id) && idLines.length < MAX_IDS) {
      ...
      if (dataNodeId) parts.push(`data-node-id="${cleanAttrValue(dataNodeId)}"`);
      if (dataEdgeId) parts.push(`data-edge-id="${cleanAttrValue(dataEdgeId)}"`);
    ...
    // knowledge-graph widgets use `[data-node-id="n1"]` / `[data-edge-id="n1-n2"]`
