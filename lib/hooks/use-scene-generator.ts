  ...
  const retrySingleOutline = useCallback(
    async (outlineId: string) => {
      ...
      try {
        ...
        if (!contentResult.success || !contentResult.content) {
          const errorMsg = contentResult.error || 'Content generation failed';
          store.getState().addFailedOutline(outline, errorMsg, contentResult.errorCode, 'content');
        ...
        if (!actionsResult.success || !actionsResult.scene) {
          const errorMsg = actionsResult.error || 'Actions generation failed';
          store.getState().addFailedOutline(outline, errorMsg, actionsResult.errorCode, 'actions');
          onSceneFailedRef.current?.(outline, errorMsg);
          return;
        }
        ...
        // courseFormat override: 'text-only' forces TTS off regardless of settings
        if (
          isTTSEnabledForFormat(state.stage?.courseFormat, settings) &&
          settings.ttsProviderId !== 'browser-native-tts' &&
          isTTSProviderEnabled(
            settings.ttsProviderId,
            settings.ttsProvidersConfig?.[settings.ttsProviderId],
          ...
          if (!ttsResult.success) {
            const errorMsg = ttsResult.error || 'TTS generation failed';
            // TTS result type doesn't carry an errorCode (unlike content/actions results);
            // pass undefined so the store still records the step + human-readable reason.
