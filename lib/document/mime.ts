...
const DOCUMENT_FORMATS: readonly DocumentFormat[] = [
  ...
  // ─── Structured data formats (local parsing, no external provider) ───
  {
    id: 'csv',
    mime: 'text/csv',
    extensions: ['.csv'],
    aliasMimes: ['text/comma-separated-values'],
    label: 'CSV',
  },
  {
    id: 'json',
  ...
  },
  {
    ...
    extensions: ['.yaml', '.yml'],
    aliasMimes: ['text/yaml', 'application/x-yaml'],
    label: 'YAML',
  },
  {
    id: 'xml',
  ...
  // ─── Web & e-book formats ───
  { id: 'html', mime: 'text/html', extensions: ['.html', '.htm'], label: 'HTML' },
  { id: 'epub', mime: 'application/epub+zip', extensions: ['.epub'], label: 'EPUB' },
  // ─── Code & notebook formats ───
  { id: 'ipynb', mime: 'application/x-ipynb+json', extensions: ['.ipynb'], label: 'IPYNB' },
  { id: 'python', mime: 'text/x-python', extensions: ['.py', '.pyw'], label: 'PY' },
  {
    id: 'javascript',
    mime: 'text/javascript',
    extensions: ['.js', '.mjs', '.cjs'],
    aliasMimes: ['application/javascript'],
    label: 'JS',
  },
  { id: 'typescript', mime: 'text/x-typescript', extensions: ['.ts', '.tsx'], label: 'TS' },
  // ─── Academic & reference formats ───
  {
    id: 'latex',
    mime: 'application/x-latex',
    extensions: ['.tex', '.ltx'],
    aliasMimes: ['text/x-latex'],
    label: 'LaTeX',
  },
  { id: 'bibtex', mime: 'application/x-bibtex', extensions: ['.bib'], label: 'BibTeX' },
  { id: 'ris', mime: 'application/x-research-info-systems', extensions: ['.ris'], label: 'RIS' },
] as const;
 ...
 * formats entirely in-process (no external API). Covers CSV/JSON/YAML/XML,
 * HTML/EPUB, IPYNB/Python/JS/TS, LaTeX/BibTeX/RIS.
 */
export const NOVA_LOCAL_MIMES: readonly string[] = [
  M.csv,
  M.json,
  M.yaml,
  M.xml,
