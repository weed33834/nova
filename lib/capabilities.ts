...
export const CAPABILITIES: Record<CapabilityId, CapabilityInfo> = {
  charts: {
    id: 'charts',
    label: 'Data Charts',
    description: 'Charts for usage dashboard and slide elements (ECharts)',
    deps: ['echarts'],
  ...
  codeHighlight: {
    ...
    description: 'Syntax highlighting for code blocks in slides (Shiki)',
    deps: ['shiki'],
  ...
  math: {
    id: 'math',
    label: 'Math Formulas',
    description: 'Math formula rendering in slides and quizzes (KaTeX / Temml)',
    deps: ['katex', 'temml'],
    downgradeable: true, // 已改造：动态加载 + 未安装降级
  ...
  agentFramework: {
    ...
    description: 'Multi-agent discussion orchestration (LangGraph)',
  ...
  docParse: {
    ...
    description: 'PDF / Office document extraction (Alibaba DocMind, unpdf)',
  ...
  objectStorage: {
    ...
    description: 'S3-compatible object storage uploads (AWS SDK)',
  ...
  zipExport: {
    id: 'zipExport',
    label: 'Zip Export',
    description: 'Export course / assets as a zip archive (JSZip)',
    deps: ['jszip'],
    downgradeable: true, // 已改造：动态加载 + 未安装降级
  ...
  fonts: {
    ...
    description: 'Additional multilingual / decorative fonts (beyond core CJK+Latin)',
    deps: [
