...
Interactive concept maps showing relationships between ideas — prerequisites, hierarchies, and dependencies.

**Best for:**
- Knowledge structure: "How concepts in a subject relate to each other"
- Prerequisite chains: "What must be learned before X"
- Concept taxonomies: "Classification hierarchies in biology, chemistry, etc."
- Causal relationship maps: "How historical events connect"
- Subject overviews: "The big picture of a discipline"

**Output in widgetOutline:**
- `graphSubject`: The subject domain (e.g., "photosynthesis", "World War II")
- `graphLayout`: "force" (default) | "hierarchy" | "radial"

**Design Principles:**
- Force-directed layout with pan & zoom
- Nodes color-coded by category (prerequisite/core/advanced/application/assessment)
- Edges labeled with relation type (requires/enhances/is_a/part_of/related_to/assesses)
- Click node → show details in info panel
- Max 15-20 nodes to keep the graph focused and readable

**When to choose knowledge-graph vs diagram:**
- Use `knowledge-graph` when the focus is on **concept relationships and prerequisite structure** (semantic edges, learning paths)
- Use `diagram` when the focus is on **processes and workflows** (sequential steps, flow)
...
| Concept relationships & prerequisites | knowledge-graph | Visualize how ideas connect |
| Subject overview / knowledge structure | knowledge-graph | Big-picture concept map |
...
## Example: Knowledge Graph Outline
...
```json
{
  ...
  "type": "interactive",
  "title": "光合作用知识图谱",
  "description": "以概念图形式展示光合作用相关概念的前置关系与分类",
  "keyPoints": ["光反应与暗反应的关系", "叶绿体的结构基础", "光合作用与呼吸作用的对比"],
  "order": 4,
