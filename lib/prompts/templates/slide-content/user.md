...
6. **Use SINGLE QUOTES for all HTML attribute values** (`<p style='font-size:20px;'>`) — NEVER use double quotes inside `content` strings, and never put bare `"` characters in the text itself (use `&quot;` or `「」` instead). This keeps the JSON valid.
