...
## Role guidelines (now snippet-based)

Role-conditional text previously lived as a TS template literal (`ROLE_GUIDELINES`
in `prompt-builder.ts`). It has been migrated to three snippet files so the
text is editable without recompiling:

| Role | Snippet ID | File |
|---|---|---|
| teacher | `role-guidelines-teacher` | `lib/prompts/snippets/role-guidelines-teacher.md` |
| assistant | `role-guidelines-assistant` | `lib/prompts/snippets/role-guidelines-assistant.md` |
| student | `role-guidelines-student` | `lib/prompts/snippets/role-guidelines-student.md` |

`loadRoleGuideline(role)` in `prompt-builder.ts` maps the incoming `agentConfig.role`
to the corresponding snippet ID and calls `loadSnippet()`. Unrecognised roles
fall back to the student guideline.

## Template metadata sidecars

Every template directory ships a `config.json` sidecar consumed by
`PromptRegistry` and the `/api/prompts` endpoint:

```json
{
  "version": "1.0.0",
  "description": "Generate slide content (titles, bullets, layout hints).",
  "tags": ["slide", "content"],
  "deprecated": false
}
```

`deprecated: true` marks a template as superseded but kept for compatibility.
The registry surfaces the metadata so tooling can filter or warn at load time.
