...
export function useSceneSelector<T = unknown, R = unknown>(selector: (data: T) => R): R {
  ...
  const readSelectedSnapshot = useCallback(() => {
    ...
    // Returning the prior reference keeps useSyncExternalStore snapshots stable,
    // including during hydration when selectors construct equivalent objects.
  ...
  return useSyncExternalStore(subscribe, readSelectedSnapshot, readSelectedSnapshot);
}
