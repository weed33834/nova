...
import type { SceneOutline, CourseFormat } from '@/lib/types/generation';
...
export async function generateMediaForOutlines(
  ...
  // Derive effective media flags from courseFormat + global settings.
  // courseFormat acts as a macro override: 'text-only' forces everything off,
  // 'ppt-audio' forces video off — regardless of global settings.
  const effectiveFlags = getEffectiveMediaFlags(courseFormat, settings);
  ...
  for (const outline of outlines) {
    ...
    for (const mg of outline.mediaGenerations) {
      // Filter by effective flags (courseFormat override applied)
      if (mg.type === 'image' && !effectiveFlags.imageEnabled) continue;
      if (mg.type === 'video' && !effectiveFlags.videoEnabled) continue;
...
export async function retryMediaTask(elementId: string): Promise<void> {
  ...
  // Check if the corresponding generation type is still enabled, respecting
  // the per-course courseFormat override (same gate as generateMediaForOutlines)
  const settings = useSettingsStore.getState();
  const courseFormat = useStageStore.getState().stage?.courseFormat;
  if (task.type === 'image' && !isImageEnabledForFormat(courseFormat, settings)) {
    store.markFailed(elementId, 'Generation disabled', 'GENERATION_DISABLED');
    return;
  }
  if (task.type === 'video' && !isVideoEnabledForFormat(courseFormat, settings)) {
    store.markFailed(elementId, 'Generation disabled', 'GENERATION_DISABLED');
    return;
  }
