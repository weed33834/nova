      ...
      if (config.providerId !== 'openai') {
        ...
        const compatFetch = async (url: RequestInfo | URL, init?: RequestInit) => {
          ...
          if (init?.body && typeof init.body === 'string') {
            try {
              const parsedBody: Record<string, unknown> = JSON.parse(init.body);
              let bodyModified = false;

              // Inject vendor-specific thinking params
              if (thinking) {
                const extra = getCompatThinkingBodyParams(providerId, config.modelId, thinking);
                if (extra) {
                  if (providerId === 'lemonade' && 'stream_options' in parsedBody) {
                    delete parsedBody.stream_options;
                  }
                  Object.assign(parsedBody, extra);
                  bodyModified = true;
                }
              }

              // Strip tool_choice when no tools are provided.
              // Some OpenAI-compatible proxies (e.g. SiliconFlow relay) reject
              // requests with tool_choice but empty/missing tools array.
              if ('tool_choice' in parsedBody) {
