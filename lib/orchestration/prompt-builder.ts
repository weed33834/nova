...
import { buildPrompt, PROMPT_IDS, loadSnippet } from '@/lib/prompts';
import type { SnippetId } from '@/lib/prompts';

// ==================== Role Guidelines ====================

/**
 * Map agent role to the snippet that carries its role guideline text.
 * Unknown roles fall back to the student guideline (shortest, safest).
 * Snippet content lives in lib/prompts/snippets/role-guidelines-<role>.md.
 */
const ROLE_SNIPPET_MAP: Record<string, SnippetId> = {
  teacher: 'role-guidelines-teacher',
  assistant: 'role-guidelines-assistant',
  student: 'role-guidelines-student',
};

/**
 * Load role guideline text from the corresponding snippet file.
 * Falls back to the student guideline for unknown roles, then to an
 * empty string if snippets are somehow unavailable (defensive).
 */
function loadRoleGuideline(role: string): string {
  const id = ROLE_SNIPPET_MAP[role] ?? 'role-guidelines-student';
  try {
    return loadSnippet(id);
  } catch {
    return '';
  }
}
...
export function buildStructuredPrompt(
  ...
  const vars = {
    ...
    roleGuideline: loadRoleGuideline(agentConfig.role),
