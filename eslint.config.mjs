...
const eslintConfig = defineConfig([
  ...
  globalIgnores([
    ...
    'packages/pptxgenjs/**',
    // Our own @nova/* packages: lint the source, but skip build output,
    // installed deps, and the vendored JS sources under importer/src1.
    'packages/@nova/*/dist/**',
    'packages/@nova/*/node_modules/**',
    'packages/@nova/importer/src1/**',
  ...
  // Package boundary (machine-enforced): @nova/renderer is a standalone,
  ...
  // isolation (only `@nova/dsl` + declared peers external), not by this rule.
  {
    files: ['packages/@nova/renderer/**/*.{ts,tsx,js,jsx,mjs,cjs}'],
    rules: {
      'no-restricted-syntax': [
        'error',
        {
          ...
          message:
            '@nova/renderer must not reference a host-app path (@/�?. This package authors no `@/…` strings �?depend only on @nova/dsl and declared peers, and inject host concerns (stores, undo, media resolution, i18n, hotkeys) via props/callbacks.',
        },
        {
          selector: 'TemplateElement[value.cooked=/^@\\//]',
          message:
            '@nova/renderer must not reference a host-app path (@/�? in a template literal. Depend only on @nova/dsl and declared peers; inject host concerns via props/callbacks.',
        },
      ],
    },
  },
  // Package boundary (machine-enforced): @nova/storage is a standalone,
  // app-agnostic persistence package. Same policy as the @nova/renderer
  ...
  // It depends only on @nova/dsl; host wiring (which store persists where)
  // lives in the app, which imports the package, never the reverse.
  {
    files: ['packages/@nova/storage/**/*.{ts,tsx,js,jsx,mjs,cjs}'],
    rules: {
      'no-restricted-syntax': [
        'error',
        {
          selector: 'Literal[value=/^@\\//]',
          message:
            '@nova/storage must not reference a host-app path (@/�?. This package authors no `@/…` strings �?depend only on @nova/dsl. The app wires its stores through the package, not the reverse.',
        },
        {
          selector: 'TemplateElement[value.cooked=/^@\\//]',
          message:
            '@nova/storage must not reference a host-app path (@/�? in a template literal. Depend only on @nova/dsl.',
        },
      ],
    },
  },
  ...
  //      on @nova/dsl (types + the fire-and-forget partition) and relative
    ...
    rules: {
      'no-restricted-syntax': [
        'error',
        {
          selector: 'Literal[value=/^@\\//]',
          message:
            'lib/choreography must not reference a host-app path (@/�?. It authors no `@/…` strings �?depend only on @nova/dsl and relative siblings, so the exporter can interpret it in pure Node. The app and exporter import it, not the reverse.',
        },
        {
          selector: 'TemplateElement[value.cooked=/^@\\//]',
          message:
            'lib/choreography must not reference a host-app path (@/�? in a template literal. Depend only on @nova/dsl and relative siblings.',
        },
        // Import allowlist (static imports/re-exports): the ONLY permitted
        // sources are `@nova/dsl`(/subpaths), `zod`, and in-folder relatives
        ...
        {
          selector:
            'ImportDeclaration > Literal.source[value=/^(?!@nova\\/dsl(\\/|$)|zod(\\/|$)|\\.\\/).+/]',
          message:
            'lib/choreography may import only from @nova/dsl, zod, or in-folder relatives (./�?. No parent-escape (../�? into the app and no other packages �?keep it pure so the exporter runs in plain Node.',
        },
        {
          selector:
            'ExportNamedDeclaration > Literal.source[value=/^(?!@nova\\/dsl(\\/|$)|zod(\\/|$)|\\.\\/).+/]',
          message:
            'lib/choreography may re-export only from @nova/dsl, zod, or in-folder relatives (./�?.',
        },
        {
          selector:
            'ExportAllDeclaration > Literal.source[value=/^(?!@nova\\/dsl(\\/|$)|zod(\\/|$)|\\.\\/).+/]',
          message:
            'lib/choreography may re-export only from @nova/dsl, zod, or in-folder relatives (./�?.',
        },
        ...
        // can pull in a render backend at runtime.
        {
          ...
          message:
            'lib/choreography must not use dynamic import() �?it bypasses the static import allowlist. Use a top-level import from @nova/dsl, zod, or a relative sibling.',
        },
        {
    ...
    rules: {
      'no-restricted-syntax': [
        'error',
        {
          selector: 'Literal[value=/^@\\//]',
          message:
            'lib/video-export must not reference a host-app path (@/�?. Live app state enters only through the injected TimingProbe / AssetSource �?depend only on @nova/dsl, zod, ../choreography, and relative siblings, so the compiler runs in pure Node.',
        },
        {
          selector: 'TemplateElement[value.cooked=/^@\\//]',
          message:
            'lib/video-export must not reference a host-app path (@/�? in a template literal. Depend only on @nova/dsl, zod, ../choreography, and relative siblings.',
        },
        {
          selector:
            'ImportDeclaration > Literal.source[value=/^(?!@nova\\/dsl(\\/|$)|zod(\\/|$)|\\.\\/|\\.\\.\\/choreography(\\/|$)).+/]',
          message:
            'lib/video-export root files may import only from @nova/dsl, zod, ../choreography, or in-module children (./�?. A ../�?into anything but ../choreography escapes the module and is rejected.',
        },
        {
          selector:
            'ExportNamedDeclaration > Literal.source[value=/^(?!@nova\\/dsl(\\/|$)|zod(\\/|$)|\\.\\/|\\.\\.\\/choreography(\\/|$)).+/]',
          message:
            'lib/video-export root files may re-export only from @nova/dsl, zod, ../choreography, or in-module children (./�?.',
        },
        {
          selector:
            'ExportAllDeclaration > Literal.source[value=/^(?!@nova\\/dsl(\\/|$)|zod(\\/|$)|\\.\\/|\\.\\.\\/choreography(\\/|$)).+/]',
          message:
            'lib/video-export root files may re-export only from @nova/dsl, zod, ../choreography, or in-module children (./�?.',
        },
        {
          selector: 'ImportExpression',
          message:
            'lib/video-export must not use dynamic import() �?it bypasses the static import allowlist. Use a top-level import from @nova/dsl, zod, ../choreography, or a relative sibling.',
        },
        {
          selector: "CallExpression[callee.name='require']",
          message:
      ...
      'no-restricted-syntax': [
        ...
        // `../../…` escape is rejected UNLESS it is exactly `../../choreography`.
        {
          selector:
            'ImportDeclaration > Literal.source[value=/^(?!@nova\\/dsl(\\/|$)|zod(\\/|$)|\\.\\/|\\.\\.\\/\\.\\.\\/choreography(\\/|$)|\\.\\.\\/(?!\\.\\.\\/)).+/]',
          message:
            'lib/video-export passes may import only from @nova/dsl, zod, ../../choreography, or in-module relatives (./�?or a single ../�?that stays inside the module). A ../../ escape into the rest of the app is rejected.',
        },
        {
          selector:
            'ExportNamedDeclaration > Literal.source[value=/^(?!@nova\\/dsl(\\/|$)|zod(\\/|$)|\\.\\/|\\.\\.\\/\\.\\.\\/choreography(\\/|$)|\\.\\.\\/(?!\\.\\.\\/)).+/]',
          message:
            'lib/video-export passes may re-export only from @nova/dsl, zod, ../../choreography, or in-module relatives.',
        },
        {
          selector:
            'ExportAllDeclaration > Literal.source[value=/^(?!@nova\\/dsl(\\/|$)|zod(\\/|$)|\\.\\/|\\.\\.\\/\\.\\.\\/choreography(\\/|$)|\\.\\.\\/(?!\\.\\.\\/)).+/]',
