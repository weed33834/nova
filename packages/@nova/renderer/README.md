...
```bash
pnpm add @nova/renderer
# or
npm install @nova/renderer
```
...
```tsx
import { SlideCanvas, type Slide } from '@nova/renderer';
...
```tsx
import { SlideRendererProvider, SlideCanvas, useSlideContext } from '@nova/renderer';
...
### Granular components — `@nova/renderer/elements`
...
export default {
  content: [
    './src/**/*.{ts,tsx}',
    './node_modules/@nova/renderer/dist/**/*.{js,cjs}',
  ],
};
```
...
[`@nova/importer`](../importer) converts `.pptx` files to the same `Slide[]` shape, so you can do `.pptx → @nova/renderer` end-to-end.
