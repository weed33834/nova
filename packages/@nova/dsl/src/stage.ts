 ...
 *
 * The split keeps `@nova/dsl` focused on the lesson skeleton while letting the
...
export interface Stage {
  ...
  /**
   * Output format for this classroom:
   * - `video`      → full classroom with video generation (default)
   * - `ppt-audio`  → PPT + TTS, no video generation
   * - `text-only`  → text content only, no media / TTS
   * Absent on legacy classrooms; treated as `video` (default).
   */
  courseFormat?: 'video' | 'ppt-audio' | 'text-only';
}
