...
The contract keystone of the Nova SDK family. `@nova/dsl` is *pure spec* — the
...
package can depend on `@nova/dsl` without pulling in junk.

## Dependency arrows (acyclic)

```
@nova/dsl       ->  (nothing)
@nova/renderer  ->  @nova/dsl
@nova/importer  ->  @nova/dsl
@nova/exporter  ->  @nova/dsl     (reserved, future)
```

`@nova/dsl` is the only package everything else depends on, and it depends on
nothing.
...
| `stage.ts`    | The lesson skeleton: `Stage`, generic `Scene<TAction, TContent>`, `SceneType`, `StageMode`, `Whiteboard`, `VideoManifest`, `SlideContent`, `QuizContent`, `MultiAgentConfig`, `GeneratedAgentConfig`, plus `isSlideContent` / `isQuizContent` guards. `Stage.courseFormat` (optional, `'video' \| 'ppt-audio' \| 'text-only'`) is the per-course macro override that the app layers on top of this skeleton to gate image / video / TTS generation; the DSL itself only carries the field, the media-gating logic lives app-side in `lib/utils/course-format.ts`. |
...
```ts
import type { Slide, PPTElement, Action } from '@nova/dsl';
import { isTextElement, DSL_VERSION, SYNC_ACTIONS } from '@nova/dsl';
```
...
1. **JSON Schema artifacts (cross-language mirror)** — `Stage`, the default
   ...
   ```ts
   import stageSchema from '@nova/dsl/schema/stage.schema.json' with { type: 'json' };
   import sceneSchema from '@nova/dsl/schema/scene.schema.json' with { type: 'json' };
   import actionSchema from '@nova/dsl/schema/action.schema.json' with { type: 'json' };
...
2. **Pure validators (in-process boundary)** — `validate*` are hand-written,
   ...
   ```ts
   import { validateStage, validateScene, validateAction } from '@nova/dsl';
...
3. **Pure normalizers (repair boundary)** — where `validate*` *reports* on a
   ...
   ```ts
   import { normalizeElement, normalizeScene, ELEMENT_DEFAULTS } from '@nova/dsl';
...
```ts
import { DSL_VERSION, migrate, dslVersionOf, needsMigration } from '@nova/dsl';
...
outside the document, per learner, through a `RuntimeStore` (`@nova/storage`).
...
Both consumers are now wired to `@nova/dsl` and no longer vendor their own copy
of the slide types:

- **`@nova/importer`**: imports all slide types from `@nova/dsl`; vendored
  `nova/types/slides.ts` deleted. The importer emits complete DSL `Slide`
  objects directly (the old partial "draft slide" + post-fill step is gone).
- **`@nova/renderer`**: imports all slide types from `@nova/dsl`; vendored
  `types/slides.ts` deleted. `@nova/dsl` is a regular dependency, kept external
  in the rollup build so consumers share one copy. The public
  `@nova/renderer/types` surface now re-exports the DSL types.

### Roadmap

- [x] Wire `@nova/importer` to import types from `@nova/dsl` (vendored copy deleted).
- [x] Wire `@nova/renderer` to import types from `@nova/dsl` (vendored copy deleted).
...
- [ ] Reserve `@nova/exporter` as the 4th family member.
...
```ts
import type { Scene, Action } from '@nova/dsl';
...
```bash
pnpm --filter @nova/dsl build         # -> dist/ (index.js, index.d.ts, …) + dist/schema/*.json
pnpm --filter @nova/dsl build:schema  # regenerate only dist/schema/*.json
pnpm --filter @nova/dsl typecheck
pnpm --filter @nova/dsl test
```

## License

MIT, matching the rest of the family (`@nova/dsl`, `@nova/importer`,
`@nova/renderer`), so the license policy is uniform across the SDK.
