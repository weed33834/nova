...
The Nova pluggable persistence layer: small, swappable-backend primitives for
persisting app state, depending only on [`@nova/dsl`](../dsl).
...
| `StorageProvider` (from `@nova/dsl`) | the asset seam: `put(blob) → ref`, `resolve(ref) → url`, `remove(ref)` | `BrowserAssetProvider` over IndexedDB + object URLs |
...
- [x] `StorageProvider` (in `@nova/dsl`) + browser `BrowserAssetProvider`
