...
function futureSessionError(sessionId: string, row: RuntimeSession): Error {
  return new Error(
    `@nova/storage: session ${JSON.stringify(sessionId)} was written at runtime DSL ` +
...
export class BrowserRuntimeStore implements RuntimeStore {
  ...
  async createSession(init: RuntimeSessionInit): Promise<RuntimeSession> {
    ...
    await this.txRun([SESSIONS], 'readwrite', async (tx) => {
      ...
      if (existing !== undefined) {
        throw new Error(`@nova/storage: session ${JSON.stringify(stamped.id)} already exists`);
      }
  ...
  async setSessionStatus(
    ...
    await this.txRun([SESSIONS], 'readwrite', async (tx) => {
      ...
      if (!row) {
        throw new Error(`@nova/storage: no session ${JSON.stringify(sessionId)}`);
      }
  ...
  async appendRecord<TPayload extends RuntimePayload>(
    ...
    return this.txRun([SESSIONS, RECORDS], 'readwrite', async (tx) => {
      ...
      if (!row) {
        throw new Error(`@nova/storage: no session ${JSON.stringify(init.sessionId)}`);
      }
      ...
      if (session.status !== 'active') {
        throw new Error(
          `@nova/storage: cannot append to session ${JSON.stringify(init.sessionId)} with ` +
  ...
  async mergeLearner(fromLearnerKey: string, toLearnerKey: string): Promise<number> {
    ...
    if (
      ...
      throw new Error('@nova/storage: learner keys must be non-empty strings');
    }
