...
function assertValid(result: ReturnType<typeof validateStage>, label: string): void {
  ...
  throw new Error(`@nova/storage: invalid ${label}: ${detail}`);
}
...
function assertStorableScene(scene: SceneLike, stageId: string): void {
  ...
  if (typeof s.id !== 'string') {
    throw new Error(`@nova/storage: scene id must be a string, got ${JSON.stringify(s.id)}`);
  }
  if (s.stageId !== stageId) {
    throw new Error(
      `@nova/storage: scene ${JSON.stringify(s.id)} has stageId ` +
        `${JSON.stringify(s.stageId)} but belongs to document ${JSON.stringify(stageId)}`,
    );
  }
  if (typeof s.order !== 'number' || !Number.isFinite(s.order)) {
    throw new Error(
      `@nova/storage: scene ${JSON.stringify(s.id)} order must be a finite number, got ` +
...
> implements DocumentStore<TScene> {
  ...
  async saveDocument(doc: MaicDocument<TScene>): Promise<void> {
    ...
    if (isFutureVersioned(doc)) {
      throw new Error(
        `@nova/storage: refusing to save document ${JSON.stringify(doc.stage.id)} — it was ` +
    ...
    for (const scene of normalized.scenes) {
      ...
      if (seen.has(scene.id)) {
        throw new Error(
          `@nova/storage: duplicate scene id ${JSON.stringify(scene.id)} in document ${JSON.stringify(stageId)}`,
        );
      }
      seen.add(scene.id);
    }
    ...
    await this.txRun([STAGES, SCENES, OUTLINES], 'readwrite', async (tx) => {
      ...
      if (existingStage && isFutureVersioned(existingStage)) {
        throw new Error(
          `@nova/storage: refusing to overwrite document ${JSON.stringify(stageId)} — the ` +
  ...
  async putScene(stageId: string, scene: TScene): Promise<void> {
    ...
    await this.txRun([STAGES, SCENES], 'readwrite', async (tx) => {
      ...
      if (!stageRow) {
        throw new Error(
          `@nova/storage: cannot putScene into missing document ${JSON.stringify(stageId)}`,
        );
      }
      ...
      if (dslVersionOf(stageRow) !== DSL_VERSION) {
        throw new Error(
          `@nova/storage: cannot putScene into document ${JSON.stringify(stageId)} at DSL ` +
  ...
  async deleteScene(stageId: string, sceneId: string): Promise<void> {
    await this.txRun([STAGES, SCENES], 'readwrite', async (tx) => {
      ...
      if (dslVersionOf(stageRow) !== DSL_VERSION) {
        throw new Error(
          `@nova/storage: cannot deleteScene from document ${JSON.stringify(stageId)} at DSL ` +
            `version ${JSON.stringify(dslVersionOf(stageRow))} — load and save it to bring it ` +
            `to ${DSL_VERSION} first`,
        );
      }
