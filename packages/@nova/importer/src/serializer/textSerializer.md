...
本文说明 `src/serializer/textSerializer.ts` 的职责、与参考渲染器 `TextRenderer` 的对应关系，以及输出如何进入 pptxtojson 的数据模型。
...
| 超链接 | `hlinkClick` + `isAllowedExternalUrl` | 相同；本工程使用 `src/utils/urlSafety.ts`（协议校验可与参考渲染器略有差异，见该文件） |

主函数名与选项类型与参考渲染器对齐：**`renderTextBody`**、**`RenderTextBodyOptions`**（如表格 `tcTxStyle` 预留的 `cellTextColor` 等）。
...
Run 侧在合并 **`defRPr` + `rPr`** 后，若仍无字色，会回退到 **形状 `lstStyle` 当前级 `defRPr`**（与参考渲染器中注释一致：处理「空 `defRPr` 盖掉 lstStyle 色」的情况）。

## 段落与 run 的视觉效果（映射到 CSS）

- **段落**：对齐、缩进、行距（无单位 / `pt` 绝对行距）、段前段后（`pt` 或相对字号比例）、**`normAutofit`** 下的行距压缩、制表宽度（`tab-size`）。
- **绝对行距 + 段内换行**：与参考渲染器一样，用带 **固定高度** 的内层 `div` 包行，避免 CJK 行高与 CSS 不一致；无此需求时外层用 **`<p>`**，有行盒时用 **`<div>`** 作外层（避免在 `<p>` 内嵌块级结构）。
...
空格与制表：连续空格用 `&nbsp;` 策略、制表用 `white-space: pre`，与参考渲染器的意图一致。
...
- **外层段落标签**：无行盒包装时使用 **`<p>`** 以贴近 README 示例；参考渲染器的段落容器始终是 **`div`**。语义与样式等价目的一致。
- **`urlSafety`**：本工程若包含 `mailto:` 等协议，与仅 `http/https` 的参考渲染器可能不完全相同；以 `src/utils/urlSafety.ts` 为准。

若需逐行对照实现，请以 **`renderTextBody` 主循环** 与 **`runStylesToCssString`** 分别对应参考渲染器中的 **段落 `paraDiv` 段** 与 **run 的 `element.style` 段**。

## 一致性核对摘要（对照参考渲染器 `TextRenderer`）

- **合并顺序**：段落七级与参考渲染器中 `merged` 构建顺序一致（defaultTextStyle → category txStyles → master ph → layout ph → shape lstStyle → pPr）。
- **Run**：`defRPr` → `rPr`，再执行 lstStyle 字色回退；`mergeRunProps` 与参考渲染器同源（颜色来自 `solidFill` / `gradFill`，无单独 `rPr` 下直接 `schemeClr` 分支）。
- **normAutofit**：`fontScale`、`lnSpcReduction` 作用于行距与 run 字号。
- **输出差异**：仅「写 DOM」与「拼 HTML 字符串」不同；段落用 `<p>`/`<div>` 与参考渲染器全 `div` 的差异见上文。
