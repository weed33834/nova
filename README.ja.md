...
<p align="center">
  <strong>マルチエージェント型 AI 教室 · あらゆるテーマをインタラクティブな学習体験に</strong>
</p>
...
Nova はマルチエージェント型の教学プラットフォームです。トピックを入力すると、AI 教師が構造化されたカリキュラムを作成し、スライドを生成し、ナレーション原稿を書き、バーチャル教室で授業を行います。複数の AI エージェントが役割を分担します — 教師が講義をリードし、アシスタントが質問に答え、ムードメーカーが場を和ませます。

位置づけは、単なるスライドジェネレーターではなく、役割分担、安全ガードレール、ナレッジトラッキングを備えた完全な教学システムです。

## 機能

### コース生成

- **AI アウトライン生成** — トピックを知識依存関係に基づく段階的なシーンに分割
- **スライド制作** — 各シーンにタイトル、要点、フローチャートを含むスライドを生成
- **音声ナレーション** — AI 教師が複数エンジンの TTS で自然な音声により各シーンを解説
- **インタラクティブ小テスト** — 選択問題と穴埋め問題を自動生成し、リアルタイムに採点
...
| ムードメーカー | 雰囲気を調整 | 発言、ホワイトボード |

- **ロール永続化** — 10 種類の組み込みロールの名称、説明、権限をカスタマイズ可能。変更はセッション間で保持
- **ランタイム制約** — 各ロールの `max_actions` と `max_turns` をランタイムで強制
- **ディスカッション制御** — Director Graph がエージェント間の発言順序と議論フローを管理

### プロンプトエンジニアリングとガバナンス

- **24 テンプレート** — アウトライン生成、コンテンツ作成、アクション順序付け、小テスト生成をカバー
- **スニペットシステム** — ロールガイドラインとアクションタイプを Markdown スニペットとして管理。再コンパイル不要で編集可能
- **安全ガードレール** — 各生成シーンで PII 検出、毒性フィルタリング、ハルシネーションスキャンを実行
- **スキルレジストリ** — 5 つの登録スキルをホワイトリストでゲート
- **REST API** — `GET /api/prompts` でテンプレート一覧、`GET /api/skills` でスキル一覧

### インフラストラクチャ

<details>
<summary><strong>LLM プロバイダ</strong></summary>
...
| Grok | Grok-4.20 |
| Tencent Hunyuan | Hunyuan-3（プレビュー） |
| Xiaomi | MiMo v2.5 |
| OpenRouter | マルチプロバイダ集約 |
| SiliconFlow | 全モデル集約 |
| Doubao | Doubao Seed 2.x シリーズ |
| Ollama | ローカルモデル |
...
- **国際化** — 英語、簡体字中国語、繁体字中国語、日本語、韓国語、アラビア語、ポルトガル語、ロシア語（クライアント側の i18next で実装）
- **ダークモード** — サイト全体でサポート
...
データフロー: ユーザーがトピックを入力 → プロンプトエンジンがプロンプトを組み立て → LLM がコンテンツを生成 → ガードレールが安全性をスキャン → マルチエージェントオーケストレーション → インタラクティブ教室のレンダリング。状態は Zustand によりブラウザのローカルストレージに永続化されます。
...
[http://localhost:3000](http://localhost:3000) を開き、トピックを入力して開始します。

### API キーなしで体験

ホームページの **"Open Cached Demo Course"** ボタンをクリックすると、API キーなしで「人工知能入門」コースを読み込めます。
