...
import { useExportVideoTimeline } from '@/lib/export/use-export-video-timeline';
...
export function HeaderControls({
  ...
  const { exporting: isExportingTimeline, exportVideoTimeline } = useExportVideoTimeline();
  const [exportMenuOpen, setExportMenuOpen] = useState(false);
  const exportRef = useRef<HTMLDivElement>(null);

  const anyExporting = isExporting || isExportingZip || isExportingTimeline;
      ...
      {onToggleEditMode && (
        <label
          className={cn(
            ...
            mode === 'edit'
              ? 'border-pink-500/60 dark:border-pink-400/60'
              : 'border-gray-100/50 dark:border-gray-700/50',
            !canEdit && mode !== 'edit'
              ? 'opacity-60 cursor-not-allowed'
              : 'cursor-pointer hover:border-pink-400/60 dark:hover:border-pink-500/50',
          )}
        ...
        >
          ...
          <Switch
            ...
            className="data-[state=checked]:bg-pink-600 dark:data-[state=checked]:bg-pink-500"
          />
        </label>
      )}
        ...
        {exportMenuOpen && (
          <div className="absolute top-full mt-2 right-0 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg shadow-lg overflow-hidden z-50 min-w-[200px]">
            ...
            >
              <FileDown className="w-4 h-4 text-gray-400 shrink-0" />
              <div>
                <div>{t('export.videoTimeline') || 'Export Video Timeline'}</div>
                <div className="text-[11px] text-gray-400 dark:text-gray-500">
                  {t('export.videoTimelineDesc') || 'Timeline IR JSON for video rendering'}
                </div>
              </div>
            </button>
          </div>
