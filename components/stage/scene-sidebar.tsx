  ...
  const { scenes, currentSceneId, setCurrentSceneId, generatingOutlines, generationStatus } =
    useStageStore(
      useShallow((s) => ({
        ...
        setCurrentSceneId: s.setCurrentSceneId,
        generatingOutlines: s.generatingOutlines,
        generationStatus: s.generationStatus,
      })),
    );
  const failedOutlines = useStageStore.use.failedOutlines();
  const failedOutlineReasons = useStageStore.use.failedOutlineReasons();
        ...
        >
            ...
            return (
              <div
                ...
                className={cn(
                  ...
                  isActive
                    ? 'bg-pink-50 dark:bg-pink-900/20 ring-1 ring-pink-200 dark:ring-pink-700'
                  ...
                  <div className="flex items-center gap-2 max-w-full">
                    <span
                      className={cn(
                        ...
                        isActive
                          ? 'bg-pink-600 dark:bg-pink-500 text-white shadow-sm shadow-pink-500/30'
                ...
                <div className="relative aspect-video w-full rounded overflow-hidden bg-gray-100 dark:bg-gray-800 ring-1 ring-black/5 dark:ring-white/5">
                  <div className="absolute inset-0 flex items-center justify-center">
                    ...
                    {isSlide && (
                      <div
                        className={cn(
                          'absolute inset-0 bg-pink-500/0 transition-colors',
                          isActive
                            ? 'bg-pink-500/0'
          ...
          {generatingOutlines.length > 0 &&
            (() => {
              ...
              const failedReason = failedOutlineReasons[outline.id];
              ...
              return (
                <div
                  ...
                  className={cn(
                    ...
                    isActive &&
                      !isFailed &&
                      'bg-pink-50 dark:bg-pink-900/20 ring-1 ring-pink-200 dark:ring-pink-700 opacity-100',
                  )}
                >
                  {/* Scene Header */}
                  <div className="flex justify-between items-center px-2 pt-0.5">
                    ...
                    <div className="absolute inset-0 flex flex-col items-center justify-center gap-1.5">
                      {isFailed ? (
                        <div className="flex flex-col items-center gap-1 px-1">
                          ...
                          {/* P1-1: Show WHY the scene failed, so users can take action
                              (e.g., fix API key, check network) instead of blindly retrying. */}
                          {failedReason && !isRetrying && (
                            <p
                              className="text-[9px] text-red-400/80 dark:text-red-500/70 text-center line-clamp-2 leading-tight"
                              title={failedReason.reason}
                            >
                              {failedReason.reason}
                            </p>
                          )}
