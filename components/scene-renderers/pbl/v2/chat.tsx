  ...
  return (
    <div className="flex h-full flex-col overflow-hidden">
            ...
            return (
              <div key={ev.id} className="flex justify-start">
                <div className="pbl-v2-task-review-shell max-w-[90%] rounded-[22px] px-4 py-3 text-sm text-slate-800">
                  <div className="mb-1 text-[10px] uppercase tracking-wider text-pink-700">
      ...
      >
        ...
        {isRoleplay && !handoverPending && !projectCompleted && (
          <div className="mb-2 flex items-center gap-1.5 px-1 text-[11px] leading-snug text-muted-foreground/80">
            <Lightbulb className="h-3.5 w-3.5 shrink-0 text-pink-200/70" />
      ...
      {handoverPending && handoverHintPos && (
        <div
          className="pointer-events-none fixed z-50 max-w-[220px] rounded-md border border-pink-300/40 bg-slate-900/95 px-2.5 py-1.5 text-[11px] font-medium text-pink-100 shadow-[0_8px_24px_rgba(6,16,34,0.45)]"
...
function StreamingDraft({
    ...
    return (
      <div className="flex justify-start">
        <div className="pbl-v2-task-review-shell max-w-[90%] animate-in fade-in-0 rounded-[22px] px-4 py-3 text-sm text-slate-800 duration-300">
          <div className="mb-1 text-[10px] uppercase tracking-wider text-pink-700">{label}</div>
          {hasTokens ? (
            <MarkdownText
  ...
  if (status === 'eval-milestone') {
    return (
      <div className="relative animate-in fade-in-0 overflow-hidden rounded-2xl border border-pink-200/85 bg-[linear-gradient(145deg,rgba(252,250,255,0.98)_0%,rgba(238,242,255,0.94)_48%,rgba(232,250,255,0.96)_100%)] p-5 text-slate-800 shadow-[inset_0_1px_0_rgba(255,255,255,0.92),0_22px_58px_rgba(8,18,38,0.30),0_0_0_1px_rgba(219, 39, 119,0.10),0_0_42px_rgba(34,211,238,0.10)] ring-1 ring-pink-300/20 duration-300">
        <div className="pointer-events-none absolute inset-x-0 top-0 h-1 bg-gradient-to-r from-pink-500 via-indigo-500 to-cyan-400" />
        <div className="relative mb-3 flex items-center gap-2 text-xs font-semibold uppercase tracking-wider text-pink-700">
          {label}
        </div>
        {hasTokens ? (
          <MarkdownText
            content={streamingEvaluationPreview(displayDraft)}
            className="pbl-v2-light-card-markdown text-slate-700 prose-p:text-slate-700 prose-strong:text-slate-900 prose-li:marker:text-pink-500"
          />
        ) : (
          <ThinkingDots tone="violet" />
        )}
        {handover && (
          <div className="relative mt-3 border-t border-pink-200/80 pt-2 text-xs text-slate-500">
...
function RoleplayHistoryBlock({
  ...
  return (
    <div className="rounded-2xl border border-pink-300/25 bg-pink-500/[0.06]">
      <button
        type="button"
        onClick={onToggle}
        aria-expanded={open}
        className="flex w-full items-center gap-2 rounded-2xl px-4 py-3 text-left transition-colors hover:bg-pink-500/[0.10]"
      >
        <Drama className="h-4 w-4 shrink-0 text-pink-200/90" />
        <span className="flex-1 text-sm font-semibold text-pink-50">{title}</span>
        <span className="text-[11px] text-pink-200/70">{messages.length}</span>
        {open ? (
          <ChevronDown className="h-4 w-4 shrink-0 text-pink-200/80" />
        ) : (
          <ChevronRight className="h-4 w-4 shrink-0 text-pink-200/80" />
        )}
      </button>
      {open && (
        <div className="space-y-3 border-t border-pink-300/15 px-4 py-4">
          {messages.map((m) => (
    ...
    return (
      <div className="flex items-center justify-center py-3" aria-label={label}>
        <div className="flex min-w-[52%] max-w-[90%] items-center gap-3">
          <div className="h-px flex-1 bg-gradient-to-r from-transparent via-pink-200/70 to-cyan-200/70" />
          <div className="relative rounded-full border border-pink-200/70 bg-gradient-to-r from-pink-50/95 via-indigo-50/90 to-cyan-50/95 px-4 py-1.5 text-[11px] font-semibold text-pink-700 shadow-[0_12px_34px_rgba(55,48,163,0.24)]">
            <span className="absolute -left-1 top-1/2 h-2 w-2 -translate-y-1/2 rounded-full bg-pink-400 shadow-[0_0_10px_rgba(244, 114, 182,0.48)]" />
          ...
          <div className="h-px flex-1 bg-gradient-to-l from-transparent via-cyan-200/70 to-pink-200/70" />
        </div>
      </div>
    );
  }
    ...
    <div className="flex justify-end">
      <div
        className={cn(
          ...
          'bg-gradient-to-br from-primary to-pink-400 text-primary-foreground shadow-[0_14px_34px_rgba(124,92,255,0.24)]',
        )}
      >
 ...
 *  decoupled from the Nova classroom teacher avatar; changing one
...
function ThinkingDots({ tone = 'muted' }: { readonly tone?: 'muted' | 'violet' } = {}) {
  const dotClass =
    tone === 'violet'
      ? 'bg-pink-500/70 shadow-[0_0_10px_rgba(219, 39, 119,0.28)]'
      : 'bg-muted-foreground';
  return (
...
function ContinuingDots({ tone = 'muted' }: { readonly tone?: 'muted' | 'violet' } = {}) {
  ...
  const dotClass =
    tone === 'violet'
      ? 'bg-pink-500/65 shadow-[0_0_10px_rgba(219, 39, 119,0.24)]'
