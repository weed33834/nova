 ...
 * lucide-react to match the rest of Nova's icon set.
 *
  ...
  if (stage === 'roleplay') {
    return (
      <div className="ml-3 mt-2 mb-0.5 flex items-center px-1">
        <span className="inline-flex items-center gap-1.5 rounded-full bg-pink-500/20 px-2.5 py-1 ring-1 ring-pink-400/35 shadow-[0_4px_14px_rgba(124,92,255,0.25)]">
          <Drama className="h-3 w-3 text-pink-100" />
          <span className="text-[10px] font-bold uppercase tracking-[0.16em] text-pink-50">
            {t(labelKey)}
          </span>
        </span>
      </div>
...
function MilestoneNode({
  ...
  const stateIcon = (() => {
    ...
    if (milestone.status === 'active')
      return (
        <CircleDot className={cn('w-3.5 h-3.5', isRoleplay ? 'text-pink-200' : 'text-primary')} />
      );
    return (
      <Lock
        className={cn('w-3.5 h-3.5', isRoleplay ? 'text-pink-300/70' : 'text-muted-foreground')}
      />
    );
  })();
  ...
  return (
    <div
      className={cn(
        ...
        isRoleplay && 'ml-3 rounded-xl border-l-[3px] border-l-pink-400/70',
        isRoleplay &&
          !active &&
          'border-pink-400/25 bg-gradient-to-br from-pink-500/[0.15] via-pink-500/[0.07] to-fuchsia-500/[0.05] shadow-[0_8px_24px_rgba(124,92,255,0.16)]',
        isRoleplay &&
          active &&
          'border-pink-300/45 bg-gradient-to-br from-pink-500/[0.24] to-fuchsia-500/[0.10] shadow-[0_12px_32px_rgba(124,92,255,0.34)] ring-1 ring-pink-300/45',
        ...
        <div className="px-2 pb-2 pt-0.5">
          <button
            ...
            className={cn(
              ...
              'bg-gradient-to-r from-primary to-pink-400 text-primary-foreground shadow-[0_8px_22px_rgba(124,92,255,0.30)] hover:brightness-110',
      ...
      {showComplete && onCompleteTask && (
        <button
          ...
          className={cn(
            ...
            'border border-pink-100/15 bg-primary/72 text-white shadow-[0_6px_16px_rgba(157,140,255,0.16)] transition-colors hover:bg-primary/88',
            'disabled:cursor-not-allowed disabled:opacity-60',
          )}
        >
