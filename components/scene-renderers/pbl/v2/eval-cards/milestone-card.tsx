 ...
 *
 * Per PR 6 D7-B: we merge what the refactor/nova-pbl repo had as
...
export function MilestoneCard({ evaluation, handover, onContinue, className }: Props) {
  ...
  return (
    ...
    >
      ...
      <div className="pointer-events-none absolute -bottom-24 -left-16 h-48 w-48 rounded-full bg-pink-200/30 blur-3xl" />
      {/* Header */}
      <div className="relative flex items-center justify-between gap-3">
        <div className="flex items-center gap-2 text-xs font-semibold uppercase tracking-wider text-pink-700">
          <span className="flex h-7 w-7 items-center justify-center rounded-full border border-pink-200 bg-pink-100 text-pink-700">
      ...
      {/* Performance prose */}
      {performance && (
        <div className="relative border-l-2 border-pink-300 bg-white/35 py-1.5 pl-3 pr-2 text-[13px] italic text-slate-600">
          {performance}
        </div>
      )}

      {/* Handover footer */}
      <div className="relative border-t border-pink-200/80 pt-2">
        {handover ? (
            ...
            <button
              ...
              className={cn(
                ...
                consumed
                  ...
                  : 'bg-gradient-to-r from-pink-600 to-cyan-500 text-white shadow-[0_12px_28px_rgba(99,102,241,0.30),0_0_22px_rgba(34,211,238,0.18)] hover:from-pink-500 hover:to-cyan-400 hover:shadow-[0_16px_34px_rgba(99,102,241,0.38),0_0_28px_rgba(34,211,238,0.24)]',
              )}
            >
              {consumed ? (
                <>
