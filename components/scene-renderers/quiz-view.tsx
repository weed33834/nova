  ...
  return (
    <div className="w-full h-full flex flex-col items-center justify-center gap-4 relative overflow-hidden">
      ...
      <div className="absolute top-0 right-0 p-6 opacity-[0.03]">
        <PieChart className="w-52 h-52 text-pink-500" />
      </div>
      <div className="absolute bottom-0 left-0 p-6 opacity-[0.02]">
        <BookOpenText className="w-40 h-40 text-pink-500 rotate-12" />
      </div>
      ...
      <motion.div
        ...
        className="w-16 h-16 bg-gradient-to-br from-pink-100 to-pink-50 dark:from-pink-900/50 dark:to-pink-900/30 rounded-2xl flex items-center justify-center shadow-lg shadow-pink-100 dark:shadow-pink-900/30 ring-1 ring-pink-200/50 dark:ring-pink-700/50"
      >
      ...
      >
        <div className="flex items-center gap-2 text-gray-500 dark:text-gray-400">
          <div className="w-7 h-7 rounded-lg bg-pink-50 dark:bg-pink-900/30 flex items-center justify-center">
            <BookOpenText className="w-3.5 h-3.5 text-pink-500" />
          </div>
          <span>
      ...
      <motion.button
        ...
        whileTap={{ scale: 0.95 }}
        onClick={onStart}
        className="mt-1 px-8 py-2.5 bg-gradient-to-r from-pink-500 to-pink-500 text-white rounded-full font-medium shadow-lg shadow-pink-200/50 dark:shadow-pink-900/50 hover:shadow-pink-300/50 transition-shadow z-10 flex items-center gap-2"
      >
        {t('quiz.startQuiz')}
          ...
          return (
            <button
              ...
              className={cn(
                ...
                !isReview &&
                  !selected &&
                  'border-gray-200 dark:border-gray-600 hover:border-pink-200 dark:hover:border-pink-700 hover:bg-pink-50/50 dark:hover:bg-pink-900/30',
                !isReview &&
                  selected &&
                  'border-pink-400 bg-pink-50 dark:bg-pink-900/30 ring-1 ring-pink-200 dark:ring-pink-700',
                // Review states
                isReview &&
                  isCorrectOpt &&
            ...
            >
              <span
                className={cn(
                  ...
                  !isReview && selected && 'bg-pink-500 text-white',
              ...
              <span
                className={cn(
                  ...
                  !isReview &&
                    !isSelected &&
                    'bg-gray-100 dark:bg-gray-700 text-gray-500 dark:text-gray-400',
                  !isReview && isSelected && 'bg-pink-500 text-white',
                  isReview && isCorrectOpt && 'bg-emerald-500 text-white',
      ...
      {!isReview ? (
        <div className="relative">
          <textarea
            ...
            className="w-full min-h-[100px] p-3 pb-10 rounded-xl border border-gray-200 dark:border-gray-600 text-sm resize-none focus:outline-none focus:border-pink-300 dark:focus:border-pink-600 focus:ring-2 focus:ring-pink-100 dark:focus:ring-pink-900/50 transition-all disabled:bg-gray-50 dark:disabled:bg-gray-800 disabled:text-gray-500 dark:bg-gray-800/50 dark:text-gray-200 dark:placeholder:text-gray-500"
          />
          <SpeechButton
            size="sm"
            disabled={disabled}
        ...
        <div className="space-y-3">
          ...
          {result.aiComment && (
            <div className="flex items-start gap-2 px-3 py-2 rounded-lg bg-pink-50 dark:bg-pink-900/30 border border-pink-100 dark:border-pink-800">
              <Sparkles className="w-4 h-4 text-pink-500 shrink-0 mt-0.5" />
              <div>
                <p className="text-xs font-medium text-pink-600 dark:text-pink-400 mb-0.5">
                  {t('quiz.aiComment')}
                </p>
              ...
              <span className="ml-auto text-xs font-bold text-pink-600 dark:text-pink-400 shrink-0">
        ...
        <div className="flex items-start gap-3">
          <span
            className={cn(
              ...
              !isReview && 'bg-pink-100 dark:bg-pink-900/50 text-pink-600 dark:text-pink-400',
              isReview &&
                result.status === 'correct' &&
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        // P0-2: Without this catch, any unexpected error (choice grading throw,
        // state setter throw, writeSubmittedResults failure) would leave phase
        // stuck at 'grading' forever — the user sees "AI 正在批改中..." with no
        // escape. Fall back to answering so the user can retry submission.
        log.error('[quiz-view] Grading pipeline failed:', err);
        if (cancelled) return;
        setPhase('answering');
      }
    })();
            ...
            <div className="flex items-center justify-between px-6 py-3 border-b border-gray-100 dark:border-gray-700 bg-white/80 dark:bg-gray-900/80 backdrop-blur shrink-0">
              ...
              <button
                ...
                className={cn(
                  ...
                  allAnswered
                    ? 'bg-gradient-to-r from-pink-500 to-pink-500 text-white shadow-sm hover:shadow-md hover:shadow-pink-200/50 dark:hover:shadow-pink-900/50 active:scale-[0.97]'
