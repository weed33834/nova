...
function PBLV2Container({
  ...
  // live inside whatever element is natively fullscreened — Nova fullscreens
  ...
  // is on. NOTE: this never reads/writes Nova code — it only chooses this
  ...
  // True while the browser is in native (OS) fullscreen — Nova enters it
  // on the stage via `stageRef.requestFullscreen()`. In that mode Nova's
    ...
    <div ref={hostRef} className="h-full w-full">
      ...
      {(runtimeProject.uiPhase === 'workspace' || runtimeProject.uiPhase === 'completed') &&
        ...
        createPortal(
          ...
          // launch), so Nova's native fullscreen still shows the workspace.
  ...
  return (
    <>
      ...
      <motion.div
        ...
        transition={transition}
      >
        {/* Native (OS) fullscreen takes over the top-right slot: Nova's
            own exit bar is hidden for PBL, so this is the only click target
            to leave true fullscreen (Esc still works, handled by Nova).
            `document.exitFullscreen()` triggers Nova's own fullscreenchange
            cleanup (keyboard unlock etc.) — we never touch Nova code. The
            violet, borderless-ish treatment mirrors Nova's native control
        ...
        {nativeFullscreen ? (
          <button
            ...
            className="absolute right-4 top-4 z-40 flex h-8 w-8 items-center justify-center rounded-md border border-pink-300/35 bg-pink-500/20 text-pink-100 shadow-sm backdrop-blur transition-colors hover:bg-pink-500/30"
