      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className="absolute inset-4 pointer-events-auto bg-white/95 dark:bg-gray-800/95 backdrop-blur-2xl rounded-3xl shadow-[0_32px_80px_-20px_rgba(0,0,0,0.25)] border-2 border-pink-200/60 dark:border-pink-700/60 flex flex-col overflow-hidden z-[120] ring-4 ring-pink-100/40 dark:ring-pink-800/40"
          >
            {/* Header */}
            <div className="h-14 px-6 border-b border-gray-100 dark:border-gray-700 flex items-center justify-between shrink-0 bg-white/50 dark:bg-gray-800/50">
              <div className="flex items-center gap-3">
                <div className="w-8 h-8 rounded-xl bg-pink-100 dark:bg-pink-900/30 flex items-center justify-center text-pink-600 dark:text-pink-400">
              ...
              <div className="flex items-center gap-2">
                <AnimatePresence>
                  {viewModified && (
                    <motion.button
                      ...
                      whileTap={{ scale: 0.9 }}
                      className="p-2 text-gray-400 dark:text-gray-500 hover:text-pink-500 dark:hover:text-pink-400 hover:bg-pink-50 dark:hover:bg-pink-900/20 rounded-lg transition-colors"
                ...
                <div className="relative">
                  <motion.button
                    ...
                    whileTap={{ scale: 0.9 }}
                    className="relative p-2 text-gray-400 dark:text-gray-500 hover:text-pink-500 dark:hover:text-pink-400 hover:bg-pink-50 dark:hover:bg-pink-900/20 rounded-lg transition-colors"
                    aria-label={t('whiteboard.history')}
                    title={t('whiteboard.history')}
                  >
                    <History className="w-4 h-4" aria-hidden="true" />
                    {snapshotCount > 0 && (
                      <span className="absolute -top-0.5 -right-0.5 min-w-[16px] h-4 px-1 rounded-full bg-pink-500 text-white text-[10px] font-bold flex items-center justify-center" aria-hidden="true">
                        {snapshotCount}
                      </span>
                    )}
                  </motion.button>
                ...
                <div className="w-px h-4 bg-gray-200 dark:bg-gray-700 mx-1" aria-hidden="true" />
                <button
                  type="button"
                  onClick={onClose}
